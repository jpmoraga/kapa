// app/dashboard/DashboardV2Client.tsx
"use client";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="font-medium">{ok ? "Onboarding completo ✅" : "Onboarding incompleto ⚠️"}</div>
...
// ✅ evita que <a> navegue cuando está “bloqueado”
function guardedAnchorProps(disabled: boolean) {
  if (!disabled) return {};
  return {
...
export default function DashboardV2Client({
  ...
  async function approve(id: string) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setPendingError(typeof data === "string" ? data : JSON.stringify(data));
        return;
      }
      await refreshAll();
    } finally {
  ...
  const totals = useMemo(() => {
    ...
    const bClp = btcClp; // CLP por BTC
    const uClp = usdtClp; // CLP por USDT
    ...
    const totalBTC =
      safeBtc + (bClp ? safeClp / bClp : 0) + (bClp && uClp ? safeUsd * (uClp / bClp) : 0);
  ...
  const canOperate = onboarding.canOperate;

  // 👇 SIGUIENTE TRAMO: el JSX completo (return (...))
  return (
    <>

{process.env.NODE_ENV !== "production" && (
  <div className="k21-card p-3 text-xs text-neutral-300">
    <div className="font-medium text-neutral-100">Diagnóstico de precios</div>

    <div className="mt-2 space-y-1">
      <div className="flex items-center justify-between gap-2">
        <span className="text-neutral-400">BTC/CLP</span>
        <span className="text-neutral-200">
          {btcClpInfo?.source ?? "—"}
          {btcClpInfo?.cached ? " · cached" : ""}
          {btcClpInfo?.stale ? " · stale" : ""}
        </span>
      </div>

      <div className="flex items-center justify-between gap-2">
        <span className="text-neutral-400">USDT/CLP</span>
        <span className="text-neutral-200">
          {usdtClpInfo?.source ?? "—"}
          {usdtClpInfo?.cached ? " · cached" : ""}
          {usdtClpInfo?.stale ? " · stale" : ""}
        </span>
      </div>
    </div>
  </div>
        ...
        {/* LEFT: PORTFOLIO + ACCIONES (V2: a la derecha visualmente) */}
        <section className="rounded-2xl bg-black p-6 shadow-lg lg:col-span-2 border border-neutral-800">
        ...
        <div className="mt-6 flex flex-col gap-3">
          <a
            href={canOperate ? newMovementHref("deposit") : onboardingCtaHref()}
            className={[
              "rounded-xl px-4 py-2 font-medium",
              canOperate ? "bg-white text-black hover:opacity-90" : "bg-white/40 text-black/70 cursor-not-allowed",
            ].join(" ")}
            {...guardedAnchorProps(!canOperate)}
            onPointerDown={(e) => {
            ...
            title={canOperate ? "" : "Completa el onboarding para operar"}
          >
            Comprar ({uiAsset(activeAsset)})
          </a>

          <a
            href={canOperate ? newMovementHref("withdraw") : onboardingCtaHref()}
            className={[
              "rounded-xl border px-4 py-2",
              canOperate ? "border-red-500/30 text-red-400 hover:bg-red-500/10" : "border-red-500/20 text-red-300/50 cursor-not-allowed",
            ].join(" ")}
            {...guardedAnchorProps(!canOperate)}
            onPointerDown={(e) => {
              if (!canOperate) return;
          ...
          {isAdmin && (
            <a
              href={canOperate ? adjustHref() : onboardingCtaHref()}
              className={[
                "rounded-xl border px-4 py-2 text-sm",
                canOperate ? "border-neutral-700 hover:bg-neutral-800" : "border-neutral-800 text-neutral-400 cursor-not-allowed",
              ].join(" ")}
              {...guardedAnchorProps(!canOperate)}
              onPointerDown={(e) => {
                if (!canOperate) return;
        ...
        {/* RIGHT: MOVIMIENTOS + PENDIENTES (V2: a la izquierda visualmente) */}
        <section className="rounded-2xl bg-neutral-950 p-6 shadow-lg lg:col-span-1 border border-neutral-800">
