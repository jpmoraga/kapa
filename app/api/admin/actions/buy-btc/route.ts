...
import { executeAdminBuyBtcFromAdmin } from "@/lib/adminActions";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (confirmation.trim().toUpperCase() !== "COMPRAR") {
      throw new Error("confirmation_text_invalid");
    }

    const detail = await executeAdminBuyBtcFromAdmin({
      actorAdminUserId: admin.admin.id,
      companyId,
      spendAmountClp: String(form.get("spendAmountClp") ?? ""),
    ...
    const message = error instanceof Error ? error.message : "admin_buy_btc_failed";
    console.error("admin:buy_btc_fail", {
