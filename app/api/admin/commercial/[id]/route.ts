...
export async function POST(
  ...
  try {
    await updateCompanyCommercialStatusFromAdmin({
      companyId: id,
      actorAdminUserId: admin.admin.id,
      status: String(form.get("status") ?? ""),
      notes: String(form.get("notes") ?? ""),
      specialTermsNote: String(form.get("specialTermsNote") ?? ""),
    });
    ...
    const message = error instanceof Error ? error.message : "commercial_update_failed";
    console.error("admin:commercial_update_fail", {
