// scripts/test_buy_usdt.ts
// Dry run (default):
//   npx tsx -r dotenv/config scripts/test_buy_usdt.ts --companyId <id> --userId <id>
// Execute:
//   npx tsx -r dotenv/config scripts/test_buy_usdt.ts --execute --companyId <id> --userId <id>
// Options:
...
async function main() {
  ...
  try {
    ...
    if (!execute) {
      console.log("[DRY RUN] Would create USDT buy + approve and validate balances.");
    ...
    const idempotencyKey = `smoke-buy-usdt-${Date.now()}`;
    ...
    try {
      await prisma.treasuryMovement.create({
        data: {
          ...
          status: TreasuryMovementStatus.PENDING,
          createdByUserId: userId,
          note: "smoke buy usdt duplicate",
          internalNote: "smoke_buy_usdt_duplicate",
          idempotencyKey,
        },
      });
    } catch (e: any) {
    ...
    await prisma.priceSnapshot.create({
      data: {
        ...
        price,
        source: "manual:smoke_buy_usdt",
      },
    });

    const startingClp = new Prisma.Decimal("10000000");
    const startingUsd = new Prisma.Decimal("0");
    const systemUsd = mode === "internal" ? amount.mul(2) : new Prisma.Decimal("0");
    ...
    const expectedUsd = new Prisma.Decimal(beforeUsd?.balance ?? 0).plus(amount);
