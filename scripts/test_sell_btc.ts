// scripts/test_sell_btc.ts
// Dry run (default):
//   npx tsx -r dotenv/config scripts/test_sell_btc.ts --companyId <id> --userId <id>
// Execute:
...
async function main() {
  ...
  try {
    ...
    if (!execute) {
      console.log("[DRY RUN] Would create BTC sell + approve and validate balances.");
    ...
    const idempotencyKey = `smoke-sell-btc-${Date.now()}`;
    ...
    const startingBtc = totalSellBase.mul(2);
    const systemBtc = mode === "external" ? totalSellBase.mul(2) : new Prisma.Decimal("0");
    ...
    try {
      await prisma.treasuryMovement.create({
        data: {
          ...
          status: TreasuryMovementStatus.PENDING,
          createdByUserId: userId,
          note: "smoke sell btc duplicate",
          internalNote: "smoke_sell_btc_duplicate",
          idempotencyKey,
        },
      });
    } catch (e: any) {
    ...
    const expectedBtc = new Prisma.Decimal(beforeBtc?.balance ?? 0).minus(totalSellBase);
